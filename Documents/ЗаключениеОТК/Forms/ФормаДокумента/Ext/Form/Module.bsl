...
Процедура ПроверкаДвойнойЗаписиНаСервере()
	...
	|	УстановкаРабочейСменыСотрудникиСмены.ФИО.Представление КАК ФИОПредставление
	|ИЗ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Для Каждого СтрокаТЧ Из Объект.ITСпециаласты Цикл
			...
			Если СтрокаТЧ.ФИО = ВыборкаДетальныеЗаписи.ФИО Тогда	
				Сообщить("Сотрудник " + ВыборкаДетальныеЗаписи.ФИОПредставление + " уже записан в другой смене "  );
			КонецЕсли;
