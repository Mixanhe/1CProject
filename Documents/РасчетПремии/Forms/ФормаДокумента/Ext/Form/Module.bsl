...
Процедура ЗаполнитьСписокСотрудниковНаСервере()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НачисленияСотрудников.Премия КАК Процент,
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Для Каждого ТекСтрокаСписокСотрудников Из ТЧСписокСотрудников Цикл
			
			Если ВыборкаДетальныеЗаписи.Сотрудник = ТекСтрокаСписокСотрудников.ФИО Тогда	
				ТекСтрокаСписокСотрудников.Процент = ВыборкаДетальныеЗаписи.Процент;	
			КонецЕсли;
