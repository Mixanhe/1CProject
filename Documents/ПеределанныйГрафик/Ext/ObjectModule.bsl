...
Процедура УстановитьРабочиеДниДляНевисокостногоГода()
	...
	// установим нерабочие дни сразу, а потом просто будем их пропускать.
	Для Каждого НерабочийДень Из НерабочиеДни Цикл 
		
		Движение = Движения.ПеределанныйГрафикРаботыПредприятия.Добавить();
	...
	// цикл, который перебирает все даты в году.
	Пока ДатаНачалаОтсчета <= ДатаОкончания Цикл
		...
		Если День(ДатаНачалаОтсчета) = НерабочийДень Тогда
...
Процедура УстановитьРабочиеДниДляВисокостногоГода()
	...
	Пока ДатаНачалаОтсчета <= ДатаОкончания Цикл
		...
		Движение.Период = ДатаНачалаОтсчета;
		Движение.Дата = ДатаНачалаОтсчета;
		Движение.ДеньНедели = Формат(ДатаНачалаОтсчета,"ДФ=дддд");	
		...
		Если Флаг = Истина Тогда				
			...
			Иначе
				...
				// UDP вот тут и стоит обработать ситуацию с сокращенным рабочим днем
				// на неделе.
				Движение.СлужебныйДляСерединыНедели = ДатаНачалаОтсчета - СекундВЧасе * 24;
				Движение.День = 0;
				ДатаНачалаОтсчета = ДатаНачалаОтсчета + СекундВЧасе * 24;			
				Флаг = Ложь;
