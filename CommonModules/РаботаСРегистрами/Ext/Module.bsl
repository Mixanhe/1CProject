Процедура УстановитьПраздничныеДниГодаНаСервере(Регистратор) Экспорт
	...
	// UPD От 03.11.2020 - предпраздничные дни, например который будет 04.11.2020 
	// вторник не установлен, как сокращенный день, моя задача сейчас это исправить.
	...
	// Теперь моя задача установить сокращенные рабочие дни на неделе перед праздниками.
	// для этого сделан отдельный реквизит, СлужебныйДляСередины. В нем записана дата предыдущего дня
	// точнее предпразднечного дня. 
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ГрафикРаботыПредприятия.СлужебныйДляСерединыНедели КАК СлужебныйДляСерединыНедели
	|ИЗ
	|	РегистрСведений.ГрафикРаботыПредприятия КАК ГрафикРаботыПредприятия
	|ГДЕ
	|	ГрафикРаботыПредприятия.Регистратор = &Регистратор
	|	И ГрафикРаботыПредприятия.СлужебныйДляСерединыНедели <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Для Каждого Запись ИЗ НаборЗаписей Цикл
			...
			Если Запись.Дата = ВыборкаДетальныеЗаписи.СлужебныйДляСерединыНедели Тогда
				Запись.ВремяНачалаРаботы = Запись.СлужебныйДляСерединыНедели + СекундВЧасе * 8;
				Запись.ВремяОкончанияРаботы = Запись.СлужебныйДляСерединыНедели + СекундВЧасе * 16;
			КонецЕсли;
